...
var get_yaw_pitch_body = func (roll_deg, pitch_deg, object_yaw_deg, object_pitch_deg, yaw_offset=0) {
    ...
    var z = math_ext2.sin(object_pitch_deg);
    var a = math_ext2.cos(object_pitch_deg);

    var x = -a * math_ext2.cos(object_yaw_deg);
    var y =  a * math_ext2.sin(object_yaw_deg);

    # Convert the position in the inertial frame to the body frame
    (x, y, z) = math_ext2.rotate_to_body_zyx(x, y, z, -roll_deg, pitch_deg, 0.0);

    var result_distance_2d = math.sqrt(math.pow(x, 2) + math.pow(y, 2));

    # Calculate heading and pitch of object in the body frame
    var result_heading = -math_ext2.atan(y, x);
    var result_pitch   =  math_ext2.atan(-z, result_distance_2d);
...
var get_yaw_pitch_distance_inert = func (position_2d, position, target_position, heading, f=nil) {
    ...
    var tp = geo.Coord.new(target_position);
    var target_position_alt = tp.alt();
    tp.set_alt(position_2d.alt());

    # Calculate heading in the inertial frame
    var heading_deg = positioned.courseAndDistance(position_2d, tp)[0] - heading;
    var distance_2d = position_2d.direct_distance_to(tp);

    tp.set_alt(target_position_alt);

    # Calculate pitch and distance in the inertial frame
    if (f == nil)
        f = math_ext2.atan;
    var pitch_deg  = f(tp.alt() - position.alt(), distance_2d);
    var distance_m = position.direct_distance_to(tp);
