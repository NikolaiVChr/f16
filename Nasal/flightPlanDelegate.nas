...
var F16RouteManagerDelegate = {
    new: func(fp) {
        # if this property is set, don't build a delegate at all
        if (0 and getprop('/autopilot/route-manager/disable-route-manager'))
            return nil;

        var m = { parents: [F16RouteManagerDelegate] };
        m.flightplan = fp;
        return m;
    },
    ...
    {
        logprint(LOG_INFO, "end of flight-plan, not deactivating");
        #fgcommand("activate-flightplan", props.Node.new({"activate": 0}));
    }
};
...
var F16GPSDeleagte = {
    new: func(fp) {
        # if this property is set, don't build a delegate at all
        if (0 and getprop('/autopilot/route-manager/disable-fms'))
            return nil;

        var m = { parents: [F16GPSDeleagte], flightplan:fp, landingCheck:nil };
    ...
    currentWaypointChanged: func
    {
        ...
        {
            me.landingCheck = maketimer(2.0, me, F16GPSDeleagte._landingCheckTimeout);
            me.landingCheck.start();
        }
    }
};

registerFlightPlanDelegate(F16GPSDeleagte.new);
registerFlightPlanDelegate(F16RouteManagerDelegate.new);